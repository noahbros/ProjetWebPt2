...
const passwordRegex = /^(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$/ //Necessite au moins une lettre, un nombre et un caractere speciaux.
...
const utilisateursRegles = [
    ...
    body('mot_de_passe').matches(passwordRegex).withMessage('Le mot de passe est invalide. Il necessite au moins une lettre, un nombre, un caractère spéciaux et au minimum 8 caractères.'),
